@extends('layouts.grim.index', ['title' => 'Detail Peminjaman ' . $book_user->user->name, 'section_header' => 'Detail Peminjaman ' . $book_user->user->name])
...
<div class="row">
  ...
  <div class="col-lg-5">
    ...
    <div class="py-4">
      <a href="{{ route('operator.book-borrowers.index') }}" class="btn btn-primary">Kembali</a>
...
<script>
  $(document).ready(function() {
    $("#book-swal-show-button").click(function() {
      ...
      $.ajax({
        url: "{{ route('operator.json-book.show', $book_user->book->id) }}",
        type: "GET",
        data: {
          id: id,
          _token: token
    ...
    $("#book-approved-button").click(function(e) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          ...
          $.ajax({
            url: "{{ route('operator.json-book.approved', $book_user->id) }}",
            type: "PUT",
            data: {
              id: id,
              _token: token
    ...
    $("#book-not-approved-button").click(function(e) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          ...
          $.ajax({
            url: "{{ route('operator.json-book.not-approved', $book_user->id) }}",
            type: "PUT",
            data: {
              id: id,
              _token: token
    ...
    $("#user-swal-show-button").click(function() {
      ...
      $.ajax({
        url: "{{ route('operator.json-user.show', $book_user->user->id) }}",
        type: "GET",
        data: {
          id: id,
          _token: token
