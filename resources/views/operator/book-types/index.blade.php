@extends('layouts.grim.index', ['title' => 'Daftar Kategori Buku', 'section_header' => 'Daftar Kategori Buku'])
          ...
          <tr>
            ...
            <td>
              <button type="submit" data-toggle="modal" data-target="#book-types-show-modal"
                data-id="{{ $book_type->id }}" class="btn btn-sm btn-info mr-2 book-types-swal-show-button">
                <i class="fas fa-info-circle"></i>
              </button>
              <button type="submit" data-toggle="modal" data-target="#book-types-edit-modal"
                data-id="{{ $book_type->id }}" class="btn btn-sm btn-success mr-2 book-types-swal-edit-button">
                <i class="fas fa-edit"></i>
              </button>
              <form action="{{ route('operator.book-types.destroy', $book_type->id) }}" method="POST" class="d-inline">
                @csrf
                @method('DELETE')
