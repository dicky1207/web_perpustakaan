@extends('layouts.grim.index', ['title' => 'Detail Buku ' . $book->title, 'section_header' => 'Detail Buku ' . $book->title])

@section('content')
<div class="row">
  <div class="col-lg-7">
    <div class="card">
      <form action="{{ route('operator.books.update', $book->id) }}" method="POST" enctype="multipart/form-data" id="form_book_update">
        @method('PUT')
        @csrf
        <table class="table">
          <tr>
  ...
  <div class="col-lg-5">
    ...
    <div class="py-4">
      <a href="{{ route('operator.books.index') }}" class="btn btn-primary">Kembali</a>
...
<script>
  $(document).ready(function() {
    ...
    $('#book_update_button').click(function(e) {
      ...
      $.ajax({
        url: "{{ route('operator.books.update', $book->id) }}",
        type: "PUT",
        data: {
          id: id,
          _token: token,
